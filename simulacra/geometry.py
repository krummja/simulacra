...
class Rect(tuple):
    """A rectangle. Origin is assumed to be top-left.
    
    NOTE: This class must not be initialized directly! Use one of the
    constructor methods `from_edges` or `from_spans`!
    """
    ...
    def iter_border(self):
        for x in range(self.left + 1, self.right):
            ...
            yield Point(y, self.bottom), Direction.down
        for x in range(self.top + 1, self.bottom):
...
class IndexRect(IndexExpression):
    pass
